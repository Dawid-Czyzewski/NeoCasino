...
export const useGame = () => {
  ...
  const updateCrashBet = useCallback((newBet) => {
    const validBet = Math.max(1, Math.min(newBet, wallet))
  ...
  const canStartCrash = useCallback(() => {
    return !isSpinning && wallet >= crashBet
  }, [isSpinning, wallet, crashBet])
